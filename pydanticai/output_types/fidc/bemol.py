...
class DadosCadastrais(ValidatedModel):
    ...
    data_verificacao: date = Field(..., description="Data da última verificação dos dados.")
    gestor: str = Field(..., description="Nome do gestor do fundo.")
    nome_do_fundo: str = Field(..., description="Nome completo do FIDC.")
    tipo_anbima: str = Field(..., description="Classificação do fundo segundo a ANBIMA.")
...
class ParametrosPatrimonioLiquido(ValidatedModel):
    ...
    caixa: float = Field(..., description="Valor total em caixa e equivalentes de caixa.")
    patrimonio_liquido: float = Field(
        ..., description="Valor total do patrimônio líquido do fundo."
    )
    pdd_total: float = Field(..., description="Provisão para Devedores Duvidosos (PDD) total.")
    ...
    valor_presente_dcs: float = Field(..., description="Valor presente dos Direitos Creditórios.")
...
class ParametrosAgenteCobranca(ValidatedModel):
    ...
    faixa_de_atraso: str = Field(..., description="Faixa de dias de atraso dos créditos.")
...
class PagamentoSituacao(ValidatedModel):
    ...
    percentual: float = Field(..., description="Percentual que esta situação representa do total.")
...
class IndiceLiquidez(ValidatedModel):
    ...
    data: date = Field(..., description="Data de referência para os índices de liquidez.")
    indice_liquidez_mezanino: float = Field(
        ..., description="Índice de liquidez da cota mezanino."
    )
    indice_liquidez_senior: float = Field(..., description="Índice de liquidez da cota sênior.")
...
class IndicesFundo(ValidatedModel):
    ...
    alocacao_bruta: float = Field(..., description="Percentual de alocação bruta dos ativos.")
    alocacao_liquida: float = Field(..., description="Percentual de alocação líquida dos ativos.")
    ...
    indice_cobertura_senior: float = Field(..., description="Índice de cobertura da cota sênior.")
    indice_liquidez: float = Field(..., description="Índice geral de liquidez do fundo.")
    indice_perdas: float = Field(..., description="Índice de perdas observadas na carteira.")
