...
class ComDinheiroApiConnector(IntegrationsIndicesConnector):
    ...
    async def get_indices_cotacoes_pontos(
        self,
        ...
        indices_cotacoes: ColetaIndiceCotacaoSchemaCollection = (
            ColetaIndiceCotacaoSchemaCollection([])
        )
        ...
        if indices_nao_sinteticos:
            indices_cotacoes.extend(
                await self.__get_indices_cotacoes(
    ...
    async def __get_indices_cotacoes(
        self,
        indices_repository: IndicesRepository,
        indices: IndiceCollection,
        moeda: Moeda,
        ...
        codigos_indices: list[str] = self.__get_enconding_codigos_tratados(
    ...
    def __get_enconding_codigos_tratados(self, codigos_indices: list[str]) -> list[str]:
            ...
            for key in linha_cotacoes:
                ...
                if is_sintetico:
                    if data_referente == indice.data_inicio_coleta:
                        preco_cotacao: Decimal = Decimal(100)
                    elif data_referente == data_inicio:
                        indice_cotacao_base: IndiceCotacao | None = (
                            await indices_repository.get_cotacao_by_indice_id_e_data_referente(
                                indice_id=indice.id,
    ...
    def __get_cotacao_sintetica(
        ...
        retorno_dia: Decimal = Decimal(retorno_str) / Decimal(100)
