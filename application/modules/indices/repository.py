...
class IndicesRepository(ABC):
    ...
    @abstractmethod
    async def get_cotacao_by_indice_id_e_data_referente(
        self, indice_id: int, data_referente: date
    ) -> IndiceCotacao | None:
        raise NotImplementedError
...
class IndicesRepositoryImpl(IndicesRepository):
    ...
    async def get_cotacao_by_indice_id_e_data_referente(
        ...
        query = select(IndiceCotacao).where(
            IndiceCotacao.indice_id == indice_id
            and IndiceCotacao.data_referente == data_referente
        )
    ...
    async def insere_cotacoes_indices(
        ...
        for lote_ids in self.__get_lotes(indices_cotacoes, TAMANHO_LOTE):
            cotacoes_dicts = [
                {
